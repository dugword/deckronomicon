...
func ActionPlayFunc(state *GameState, player *player.Player, target game.ActionTarget) (result ActionResult, err error) {
	...
	if targetObj == nil {
		...
		if target.Name != "" && len(choices) == 1 {
			...
			choice, err = player.Agent.ChooseOne(
				"Which card to play",
				choose.NewChoiceSource(string(game.ActionPlay)),
