...
func ActionActivateFunc(state *GameState, player *player.Player, target game.ActionTarget) (ActionResult, error) {
	...
	choice, err := player.Agent.ChooseOne(
		...
		choose.NewChoiceSource(string(game.ActionActivate)),
