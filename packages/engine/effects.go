...
func BuildEffect(source game.Object, spec EffectSpec) (*Effect, error) {
	switch spec.ID {
	case "AdditionalMana":
...
// Keys: Count, Type
// Default: Count: 1
func BuildEffectDraw(source game.Object, spec EffectSpec) (*Effect, error) {
...
//   - Mana: <ManaString>
func BuildEffectAddMana(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	var mana string
	for _, modifier := range spec.Modifiers {
...
//   - Target: <subtype>
//   - Duration: <eventType>
func BuildEffectAdditionalMana(source game.Object, spec EffectSpec) (*Effect, error) {
	var mana string
	var target string
	var duration string
	for _, modifier := range spec.Modifiers {
	...
	subtype, err := game.StringToSubtype(target)
	if err != nil {
...
func BuildEffectCounterSpell(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	...
	var cardTypes []game.CardType
	if len(targetTypes) != 0 {
		for _, target := range targetTypes {
			cardType, err := game.StringToCardType(target)
			if err != nil {
...
func BuildEffectLookAndChoose(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	lookCount := "1"
	chooseCount := "1"
	restZone := "Library"
...
func BuildEffectMill(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	count := "1"
	var target string
	for _, modifier := range spec.Modifiers {
...
func BuildEffectPutBackOnTop(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	count := "1"
	for _, modifier := range spec.Modifiers {
		if modifier.Key != "Count" {
...
func BuildEffectScry(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	count := "1"
	for _, modifier := range spec.Modifiers {
		if modifier.Key != "Count" {
...
func BuildEffectDiscard(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	count := "1"
	var delay string
	for _, modifier := range spec.Modifiers {
...
func BuildEffectSearch(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	var filters []FilterFunc
	for _, modifier := range spec.Modifiers {
		// TODO: Could probably just have this be "type" and figure out if
...
// Supported Modifier Keys (last applies):
func BuildEffectTransmute(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	card, ok := source.(*Card)
	if !ok {
...
// Supported Modifier Keys (last applies):
//   - Subtype <subtype>
func BuildEffectTypecycling(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	var subtype string
	for _, modifier := range spec.Modifiers {
...
func BuildEffectShuffleFromGraveyard(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	count := "1"
	for _, modifier := range spec.Modifiers {
		if modifier.Key == "Count" {
...
func BuildEffectTap(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	var target string
	for _, modifier := range spec.Modifiers {
		if modifier.Key == "Target" {
...
func BuildEffectTapOrUntap(source game.Object, spec EffectSpec) (*Effect, error) {
	effect := Effect{ID: spec.ID}
	var target string
	for _, modifier := range spec.Modifiers {
		if modifier.Key == "Target" {
...
func Scry(state *GameState, source game.Object, n int, player *Player) error {
	var taken []game.Object
	for range n {
		card, err := player.Library.TakeTop()
		if err != nil {
