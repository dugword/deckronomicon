...
func ApplyEventAndTriggers(game *state.Game, gameEvent event.GameEvent) (*state.Game, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to apply event %q: %w", gameEvent.EventType(), err)
	}
	triggeredAbilities, err := CheckTriggeredAbilities(game, gameEvent)
	if err != nil {
		return nil, fmt.Errorf("failed to check triggered abilities: %w", err)
	}
	for _, triggeredAbility := range triggeredAbilities {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to handle triggered ability %q: %w", triggeredAbility.ID, err)
		}
		for _, triggeredEvent := range triggeredEvents {
			game, err = ApplyEventAndTriggers(game, triggeredEvent)
			if err != nil {
				return nil, fmt.Errorf("failed to apply triggered event %q: %w", triggeredEvent.EventType(), err)
			}
		}
	}
	...
	if gameEvent.EventType() == "BeginEndStep" {
		...
		for _, ta := range game.TriggeredAbilities() {
			if ta.Duration == mtg.DurationEndOfTurn {
				...
				if err != nil {
					return nil, fmt.Errorf("failed to remove triggered ability %q: %w", ta.ID, err)
				}
			}
		}
	}
