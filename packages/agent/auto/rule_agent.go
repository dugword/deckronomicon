...
func (a *RuleBasedAgent) GetNextAction(state player.GameState) (action.Action, error) {
	s, ok := state.(*engine.GameState)
	if !ok {
		return action.Action{}, fmt.Errorf("expected GameState, got %T", state)
	}
	...
	for _, mode := range a.strategy.Modes {
		...
		if err != nil {
			return action.Action{}, fmt.Errorf("failed to evaluate condition for mode %s: %w", mode.Name, err)
		}
		if result {
	...
	for _, rule := range a.strategy.Rules[a.player.Mode] {
		...
		if err != nil {
			return action.Action{}, fmt.Errorf("failed to evaluate condition for rule %s: %w", rule.Name, err)
		}
		if !result {
			continue
		}
		...
		if err != nil {
			return action.Action{}, fmt.Errorf("failed to resolve action for rule %s: %w", rule.Name, err)
		}
		break // Stop after the first matching rule
	}
	a.uiBuffer.UpdateFromState(s, a.player)
	if err := a.uiBuffer.Render(); err != nil {
		return action.Action{}, fmt.Errorf("failed to render UI buffer: %w", err)
	}
	if string(act.Type) == "" {
		fmt.Println("No action matched for player:", a.player.ID())
		enterToContinue()
		return action.Action{Type: action.ActionPass}, nil // No action matched, just pass
	}
