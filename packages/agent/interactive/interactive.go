...
func (a *InteractivePlayerAgent) GetNextAction(state player.GameState) (action.Action, error) {
	s, ok := state.(*engine.GameState)
	if !ok {
		return action.Action{}, fmt.Errorf("InteractivePlayerAgent.GetNextAction: state is not of type *engine.GameState")
	}
	for {
		...
		command.Action.Target = action.ActionTarget{Name: arg}
